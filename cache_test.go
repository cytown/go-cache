...
func TestStorePointerToStruct(t *testing.T) {
	tc := New[any](DefaultExpiration, 0)
	tc.Set("foo", &TestStruct[any]{Num: 1}, DefaultExpiration)
	x, found := tc.Get("foo")
	if !found {
		t.Fatal("*TestStruct was not found for foo")
	}
	foo := x.(*TestStruct[any])
	foo.Num++
...
func TestIncrementWithInt16(t *testing.T) {
	...
	err := tc.Increment("tint16", int16(2))
	if err != nil {
		t.Error("Error incrementing:", err)
	}
...
func TestDecrementWithAnyUint64(t *testing.T) {
	tc := New[any](DefaultExpiration, 0)
	tc.Set("uint64", uint64(5), DefaultExpiration)
	err := tc.Decrement("uint64", uint64(2))
	if err != nil {
		t.Error("Error decrementing:", err)
	}
	x, found := tc.Get("uint64")
...
func TestDecrementWithFloat32(t *testing.T) {
	...
	err := tc.Decrement("float32", float32(2))
	if err != nil {
		t.Error("Error decrementing:", err)
	}
	x, found := tc.Get("float32")
...
func testFillAndSerialize(t *testing.T, tc *Cache[any]) {
	...
	tc.Set("*struct", &TestStruct[any]{Num: 1}, DefaultExpiration)
	tc.Set("[]struct", []TestStruct[any]{
		{Num: 2},
		{Num: 3},
	}, DefaultExpiration)
	tc.Set("[]*struct", []*TestStruct[any]{
		&TestStruct[any]{Num: 4},
		&TestStruct[any]{Num: 5},
	}, DefaultExpiration)
	tc.Set("structception", &TestStruct[any]{
		Num: 42,
		Children: []*TestStruct[any]{
			&TestStruct[any]{Num: 6174},
			&TestStruct[any]{Num: 4716},
		},
	}, DefaultExpiration)
